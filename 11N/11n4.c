...
double result;
enum type option;  //열거형 변수 선언
scanf("%d",&option);

union P polygon; // 공용체 변수 선언

if( option == TRI ){
scanf("%lf",&polygon.T.h); scanf("%lf",&polygon.T.w);
result = (polygon.T.w*polygon.T.h)/2;
printf("%lf",result);

} else if ( option == REC ){
scanf("%lf",&polygon.R.x); scanf("%lf",&polygon.R.y);
result = polygon.R.x*polygon.R.y;
printf("%lf",result);

} else if ( option == CIR ){
scanf("%lf",&polygon.C.r);
result = pow(polygon.C.r,2)*pie;
printf("%lf",result);
